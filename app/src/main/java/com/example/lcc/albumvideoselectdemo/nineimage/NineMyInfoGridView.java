...
public class NineMyInfoGridView extends ViewGroup {
    ...
    private static NineGridView.ImageLoader mImageLoader;        //全局的图片加载器(必须设置,否者不显示图片)

    private int singleImageWidthSize = 290;              // 单张图片时的宽度最大大小,单位dp
    private int singleImageHeightSize = 290;              // 单张图片时的高度最大大小,单位dp
    ...
    public NineMyInfoGridView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NineMyInfoGridView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        DisplayMetrics dm = context.getResources().getDisplayMetrics();
        gridSpacing = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, gridSpacing, dm);
        singleImageWidthSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, singleImageWidthSize, dm);
        ...
        singleImageWidthSize = a.getDimensionPixelSize(R.styleable.NineGridView_ngv_singleImageSize, singleImageWidthSize);
...
//                gridWidth = singleImageWidthSize > totalWidth ? totalWidth : singleImageWidthSize;
//                gridHeight = (int) (gridWidth / singleImageRatio);
//                //矫正图片显示区域大小，不允许超过最大显示范围
//                if (gridHeight > singleImageWidthSize) {
//                    float ratio = singleImageWidthSize * 1.0f / gridHeight;
//                    gridWidth = (int) (gridWidth * ratio);
//                    gridHeight = singleImageWidthSize;
//                }
//            } else
                if (mImageInfo.size() == 2 || mImageInfo.size() == 4 || mImageInfo.size() == 1) {
...
//                gridWidth = gridHeight = (totalWidth - gridSpacing ) / 2;
        ...
        if (position < imageViews.size()) {
            ...
            imageView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    mAdapter.onImageItemClick(getContext(), NineMyInfoGridView.this, position, mAdapter.getImageInfo());
                }
            });
     ...
     * 设置只有一张图片时的宽
     */
    public void setSingleImageWidthSize(int maxImageSize) {
        singleImageWidthSize = maxImageSize;
    }

    public void setSingleImageHeightSize(int singleImageHeightSize) {
        this.singleImageHeightSize = singleImageHeightSize;
    }
    ...
    public static void setImageLoader(NineGridView.ImageLoader imageLoader) {
        mImageLoader = imageLoader;
    }

    public static NineGridView.ImageLoader getImageLoader() {
        return mImageLoader;
    }
